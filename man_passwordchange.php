...
$sql = "SELECT * FROM manager WHERE num='{$account}'";
$result = mysqli_query($link, $sql);
$row = mysqli_fetch_array($result);
$usedpassword=$row['mpassword'];
if(isset($_POST['password'])||isset($_POST['surepassword'])||isset($_POST['pastpassword']))
{
	...
	if($i)
	{
		...
		{
			$sql="update manager set mpassword='{$password}' where num='{$account}'";
			$result = mysqli_query($link, $sql);
			if($result )
			{
...
<button type="button" style="text_align:center" onclick="window.location='manager.php';">返回</button>
