...
if(isset($_POST['password'])||isset($_POST['surepassword'])||isset($_POST['pastpassword']))
{
	...
	if($i)
	{
		...
		{
			$sql="update student set spassword='{$password}' where sno='{$account}'";
			$result = mysqli_query($link, $sql);
			if($result )
			{
...
<button type="button" style="text_align:center" onclick="window.location='student.php';">返回</button>
